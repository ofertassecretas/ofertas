...
def construir_roteiro_moto():
    r=[];u=set()
    for p in MOTO_CICLOS_PRIORITARIOS:
        if p not in u:r.append(p);u.add(p)
    for peca in PECAS_MOTO:
        for moto in MOTOS:
            p=(moto,peca)
            if p not in u:r.append(p);u.add(p)
    return r
...
def carregar_estado():
    try:
        with open(ESTADO_FILE,"r",encoding="utf-8") as f:estado=json.load(f) if os.path.exists(ESTADO_FILE) else {}
    except:estado={}
    estado.setdefault("Moto",{})
    for n in ["Casa","Maternidade","Eletroeletrônicos","Moda feminina","Moda masculina"]:estado.setdefault(n,{})
    estado.setdefault("free_nicho_idx",0)
    if estado.get("rodizio_buscas_versao",0)!=RODIZIO_BUSCAS_VERSAO:
        logging.info(f"Atualizando rodízio para versão {RODIZIO_BUSCAS_VERSAO}")
        estado["Moto"]["roteiro_idx"]=0;estado["Moto"]["pares"]=[];estado["Moto"]["pares_idx"]=0
        for n in ["Casa","Maternidade","Eletroeletrônicos","Moda feminina","Moda masculina"]:
            estado[n]["resultado_idx"]={};estado[n]["produtos_ordem"]=list(range(len(PRODUTOS_NICHO[n])));estado[n]["produto_idx"]=0
        estado["rodizio_buscas_versao"]=RODIZIO_BUSCAS_VERSAO
    else:
        estado["Moto"].setdefault("roteiro_idx",0)
        for n in ["Casa","Maternidade","Eletroeletrônicos","Moda feminina","Moda masculina"]:
            estado[n].setdefault("resultado_idx",{});estado[n].setdefault("produtos_ordem",list(range(len(PRODUTOS_NICHO[n]))));estado[n].setdefault("produto_idx",0)
            if len(estado[n]["produtos_ordem"])!=len(PRODUTOS_NICHO[n]):estado[n]["produtos_ordem"]=list(range(len(PRODUTOS_NICHO[n])));estado[n]["produto_idx"]=0
    return estado

def salvar_estado(estado):
    try:
        with open(ESTADO_FILE,"w",encoding="utf-8") as f:json.dump(estado,f,ensure_ascii=False)
    except Exception as e:logging.error(f"Erro salvando estado: {e}")
...
def salvar_historico(hist):
    try:
        with open(HISTORICO_FILE,"w",encoding="utf-8") as f:json.dump(hist,f,ensure_ascii=False)
    except Exception as e:logging.error(f"Erro salvando historico: {e}")

def dentro_do_horario():return dt_time(5,30)<=datetime.now(FUSO_BR).time()<=dt_time(21,30)

def normalizar_texto(txt):
    if not txt:return ""
    txt=txt.lower().strip();txt=re.sub(r"[^a-z0-9à-ÿ\s]"," ",txt);return re.sub(r"\s+"," ",txt)

def chave_base_titulo(titulo):
    t=normalizar_texto(titulo);stop={"premium","novo","promocao","promoção","super","original","profissional","casual","masculino","feminino","infantil","adulto","unissex","estica","kit","com","de","para","o","a","promo","oferta","modelo","versao","versão","linha","envio","usado","branco","preto","azul","vermelho","rosa","verde","amarelo","tamanho","tamanhos","unico","único","gamer","led","usb"};return " ".join(sorted([x for x in t.split() if x not in stop and len(x)>2])[:8])

def tem_bloqueio(titulo):
    t=normalizar_texto(titulo);p=["teste","amostra","não compre","nao compre","produto teste","exemplo","dummy","vela led","vela decorativa","decorativa","decoração","casamento","festa"];return any(x in t for x in p)

def titulo_duplicado_forte(titulo):
    t=normalizar_texto(titulo);base=chave_base_titulo(titulo)
    for prev in ULTIMOS_TITULOS:
        if t==prev or SequenceMatcher(None,t,prev).ratio()>=SIMILARIDADE_MAX or (base and base==chave_base_titulo(prev)):return True
    return False

def shop_type_score(shop_type):
    try:
        if not shop_type:return 0
...
def motivo_rejeicao(p):
    try:
        titulo=str(p.get("productName","")).strip();link=str(p.get("offerLink") or p.get("productLink") or "").strip();preco=float(p.get("priceMin",0) or 0);comissao=float(p.get("commissionRate",0) or 0);vendas=int(p.get("sales",0) or 0);rating=float(p.get("ratingStar",0) or 0)
        ...
        return None
    except Exception as e:return f"erro_validacao:{type(e).__name__}"

def historico_bloqueia(chave):
    hist=carregar_historico()
    if chave not in hist:return False
    try:return datetime.now(FUSO_BR)-datetime.fromisoformat(hist[chave])<timedelta(days=HISTORICO_DIAS)
    except:return False

def registrar_historico(chave):
    hist=carregar_historico();hist[chave]=datetime.now(FUSO_BR).isoformat();salvar_historico(hist)
...
def montar_catalogo():return {n:[(p,"","") for p in ps] for n,ps in PRODUTOS_NICHO.items()}
CATALOGOS=montar_catalogo()

def get_proximo_termo(nicho,estado):
    catalogo=CATALOGOS[nicho];st=estado.setdefault(nicho,{});ordem=st.get("produtos_ordem",list(range(len(catalogo)))) or list(range(len(catalogo)));idx=st.get("produto_idx",0);pos=ordem[idx%len(ordem)];st["produto_idx"]=(idx+1)%len(ordem);return catalogo[pos],estado

def get_proxima_combinacao_moto(estado):
    st=estado.setdefault("Moto",{});idx=st.get("roteiro_idx",0);par=MOTO_ROTEIRO[idx%len(MOTO_ROTEIRO)];st["roteiro_idx"]=(idx+1)%len(MOTO_ROTEIRO);logging.info(f"RODIZIO MOTO -> índice={idx} | peça={par[1]} | modelo={par[0]}");return par,estado
...
def selecionar_ofertas_termo(nicho,termo,cota,estado,e_moto=False,peca=None):
    ...
    kw=termo if not e_moto else f"{peca} {termo}";resultados=buscar_produtos_da_categoria_kw(kw,nicho);filtrados=[];rejeitados_local=Counter()
    for p in resultados:
        motivo=motivo_rejeicao(p)
        if motivo is None:filtrados.append(p)
        else:rejeitados_local[motivo]+=1;REJEICOES[motivo]+=1
    ...
    if len(escolhidos)<cota:logging.warning(f"{nicho}: só conseguiu {len(escolhidos)}/{cota}")
    return escolhidos,estado

def get_shopee_offers():
    global usados_no_ciclo,BASES_VISTAS
    usados_no_ciclo=set();BASES_VISTAS=set();candidatos=[];estado=carregar_estado();ordem_nichos=["Moto","Casa","Maternidade","Eletroeletrônicos","Moda feminina","Moda masculina"];cotas={"Moto":2,"Casa":2,"Maternidade":2,"Eletroeletrônicos":2,"Moda feminina":1,"Moda masculina":1}
    for nicho in ordem_nichos:
        try:
            if nicho=="Moto":
                for _ in range(cotas[nicho]):
                    (moto,peca),estado=get_proxima_combinacao_moto(estado);escolhidos,estado=selecionar_ofertas_termo(nicho,moto,1,estado,e_moto=True,peca=peca);candidatos.extend((nicho,p) for p in escolhidos)
            else:
                for _ in range(cotas[nicho]):
                    (termo,_,_),estado=get_proximo_termo(nicho,estado);escolhidos,estado=selecionar_ofertas_termo(nicho,termo,1,estado);candidatos.extend((nicho,p) for p in escolhidos)
        except Exception as e:logging.error(f"Erro no nicho {nicho}: {e}",exc_info=True)
    salvar_estado(estado);candidatos.sort(key=lambda x:oferta_score(x[1]),reverse=True);logging.info(f"Shopee OK: {len(candidatos[:MAX_OFERTAS])} produtos exclusivos para envio");return candidatos[:MAX_OFERTAS]
...
def gerar_link_whatsapp_from_html(msg_html):return f"https://wa.me/?text={quote(re.sub(r'<[^>]+>','',msg_html))}"

def aplicar_id_afiliado(link):
    parsed=urlparse(link);query=parse_qs(parsed.query);query["af_siteid"]=AFILIADO_ID;return urlunparse(parsed._replace(query=urlencode(query,doseq=True)))
...
async def send_ofertas(context:ContextTypes.DEFAULT_TYPE):
    try:
        ...
        usadas_abertura.clear();usadas_gatilho.clear();shopee_ofertas=get_shopee_offers();selecionadas=[]
        if len(shopee_ofertas)<MIN_OFERTAS:logging.warning(f"Apenas {len(shopee_ofertas)} ofertas válidas. Pulando envio.");return
        for nicho_origem,item in shopee_ofertas[:MAX_OFERTAS]:
            try:
                link_base=item.get("offerLink") or item.get("productLink");link=aplicar_id_afiliado(link_base);nome=html.escape(item["productName"]);preco=float(item["priceMin"]);img=item["imageUrl"];rating=float(item.get("ratingStar",4.5));vendas=int(item.get("sales",100));comissao=round(float(item.get("commissionRate",0))*100,2);assinatura=f"{item.get('productName','')}|{link_base or ''}";produto_id=hashlib.md5(assinatura.encode()).hexdigest();vendas_f=f"{vendas:,}".replace(",",".");preco_f=f"{preco:.2f}".replace(".",",");msg=gerar_copy(nome,preco_f,vendas_f,rating,comissao,link,False);zap_msg=gerar_copy(nome,preco_f,vendas_f,rating,0,link,True);zap=gerar_link_whatsapp_from_html(zap_msg);msg+=f'\n📲 <a href="{zap}">Compartilhar no WhatsApp</a>\n━━━━━━━━━━━━━━━\n📢 <b>Ofertas Secretas</b>';selecionadas.append({"msg":msg,"img":img,"produto_id":produto_id,"item_raw":item,"nicho_origem":nicho_origem})
            except Exception as e:logging.error(f"Erro Shopee item: {e}",exc_info=True)
        logging.info(f"Selecionadas: {len(selecionadas)}")
        if not selecionadas:logging.warning("Nenhuma oferta encontrada");return
        ...
        for item in selecionadas:
            try:
                logging.info(f"Enviando produto para VIP (nicho {item['nicho_origem']})");await context.bot.send_photo(chat_id=CHAT_ID_DESTINO,photo=item["img"],caption=item["msg"],parse_mode="HTML");registrar_historico(item["produto_id"]);await asyncio.sleep(40)
            except Exception as e:logging.error(f"Erro Telegram VIP: {e}",exc_info=True)
        logging.info("=== ENTROU NO BLOCO GRATUITO ===");estado=carregar_estado();idx=estado.get("free_nicho_idx",0);nicho_alvo=NICHOS_FREE_ROTA[idx%len(NICHOS_FREE_ROTA)];logging.info(f"Rodízio FREE, nicho alvo: {nicho_alvo}");oferta_free=next((x for x in selecionadas if x["nicho_origem"]==nicho_alvo),None)
        if oferta_free is None:logging.warning(f"Não encontrei oferta do nicho {nicho_alvo} neste ciclo para o FREE.")
        else:
            try:logging.info(f"Enviando oferta para FREE (nicho {nicho_alvo})");await context.bot.send_photo(chat_id=FREE_CHAT_ID,photo=oferta_free["img"],caption=oferta_free["msg"],parse_mode="HTML");registrar_historico(oferta_free["produto_id"])
            except Exception as e:logging.error(f"Erro Telegram FREE: {e}",exc_info=True)
        estado["free_nicho_idx"]=(idx+1)%len(NICHOS_FREE_ROTA);salvar_estado(estado);logging.info("Loop finalizado")
    except Exception as e:logging.error(f"ERRO CRITICO: {e}",exc_info=True)

async def keep_alive():
    while True:logging.info("BOT VIVO");await asyncio.sleep(300)

async def post_init(app):
    app.job_queue.run_repeating(send_ofertas,interval=CHECK_INTERVAL,first=10);asyncio.create_task(keep_alive());logging.info("🤖 BOT RODANDO ESTAVEL")
...
if __name__=="__main__":
    if not TELEGRAM_TOKEN:raise RuntimeError("TELEGRAM_TOKEN ausente")
    while True:
        try:
