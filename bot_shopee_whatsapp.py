...
MAX_OFERTAS = 10
MIN_OFERTAS = 4
HISTORICO_DIAS = 30               # Bloqueia produto por 30 dias
HISTORICO_PERMANENTE = True       # Regra de histórico prolongada
SIMILARIDADE_MAX = .88
...
PRECO_MAX = 10000
COMISSAO_MIN = .03
RODIZIO_BUSCAS_VERSAO = 10        # Nova versão = reseta estado
LIMITE_POR_FAMILIA = 1            # Apenas 1 produto por família por ciclo
BUSCA_PAGINAS_MAX = 4             # Busca até a página 4
TIPOS_ORDEM = [1, 2, 3, 4, 5]     # 1=Relevância,2=MaisVendidos,3=MaiorDesconto,4=MenorPreço,5=MaiorPreço
...
def gerar_variacoes_termo(termo):
    """Gera variações para trazer produtos diferentes a cada busca"""
    termo_base = termo.strip()
    variacoes = [
        termo_base,
        f"{termo_base} promocao",
        f"{termo_base} oferta",
        f"{termo_base} novo",
        f"{termo_base} original",
        f"{termo_base} barato",
        f"{termo_base} premium",
        f"{termo_base} 2025",
    ]
...
def salvar_json_seguro(arq, dados):
    try:
        ...
        logging.error("Erro salvando %s: %s", arq, e)
...
def carregar_json(arq, padrao):
    try:
        ...
        logging.error("Erro lendo %s: %s", arq, e)
        return padrao
...
def carregar_estado():
    ...
    if e.get("rodizio_buscas_versao") != RODIZIO_BUSCAS_VERSAO:
        ...
        for n, l in PRODUTOS_NICHO.items():
            e[n] = {"pos": 0, "data_rodizio": hoje}
    e.setdefault("Moto", {"data_rodizio": "", "idx": 0})
    e["Moto"].setdefault("idx", 0)
    e["Moto"].setdefault("data_rodizio", "")
    for n in PRODUTOS_NICHO:
        e.setdefault(n, {"pos": 0, "data_rodizio": ""})
        e[n].setdefault("pos", 0)
        e[n].setdefault("data_rodizio", "")
    e.setdefault("free_nicho_idx", 0)
    return e

def salvar_estado(e):
    salvar_json_seguro(ESTADO_FILE, e)

def carregar_historico():
    return carregar_json(HISTORICO_FILE, {})

def salvar_historico(h):
    salvar_json_seguro(HISTORICO_FILE, h)
...
def get_proximo_termo(nicho, e):
    l = PRODUTOS_NICHO[nicho]
    st = e[nicho]
    for _ in range(len(l)):
        termo = l[st["pos"] % len(l)]
        st["pos"] += 1
...
def shop_score(s):
    try:
        return 3 if 1 in s else 2 if 4 in s else 1 if 2 in s else 0
    except:
        return 0
...
def oferta_score(p, termo=""):
    try:
        ...
        tn = normalizar_texto(termo)
        s = min(v / 8, 25) + r * 2 + c * 100 + shop_score(p.get("shopType", []))
        if 50 <= pr <= 5000:
            s += 6
        if tn:
            s += 8 if tn in n else sum(2 for x in tn.split() if x in n)
        return s
    except:
        return 0
...
def historico_bloqueia(chave):
    ...
    try:
        ...
        dias_passados = datetime.now(FUSO_BR) - d
        if HISTORICO_PERMANENTE:
            return dias_passados < timedelta(days=HISTORICO_DIAS)
        else:
            return dias_passados < timedelta(days=HISTORICO_DIAS)
    except:
        return False
...
def validar_relevancia(nicho, titulo, termo=None, modelo=None, peca=None):
    ...
    if nicho == "Eletroeletrônicos" and any(x in t for x in ["capa", "pelicula"]) and not any(x in t for x in ["celular", "tablet", "iphone"]):
        return False
    if nicho == "Casa" and "tinta" in t and not any(x in t for x in ["parede", "spray"]):
        return False
    ...
    if nicho == "Maternidade" and any(x in t for x in ["organizador", "cozinha"]) and not any(x in t for x in ["bebe", "infantil"]):
        return False
    if nicho == "Moto" and ((modelo and not validar_modelo(titulo, modelo)) or (peca and not validar_peca(titulo, peca))):
        return False
    return True
...
def buscar_produtos(kw, nicho):
    ...
    try:
        r = requests.post(SHOPEE_GRAPHQL_URL, data=payload.encode(), headers=headers, timeout=25)
        r.raise_for_status()
        d = r.json()
        if d.get("errors"):
            logging.error("GraphQL: %s", d["errors"])
            return []
        p = d.get("data", {}).get("productOfferV2", {}).get("nodes", []) or []
        logging.info("✅ Retornou %s produtos (página %s, ordem %s)", len(p), pagina, ordem)
        return p
    except Exception as e:
...
def get_shopee_offers():
    ...
    salvar_estado(e)
    candidatos.sort(key=lambda x: oferta_score(x[1], x[0]), reverse=True)
...
async def enviar_lote(ctx, lista):
    ...
    for item in lista:
        ...
        if await enviar_produto(ctx, item, CHAT_ID_DESTINO):
...
async def send_ofertas(ctx):
    try:
        ...
        if len(ofertas) < MIN_OFERTAS:
            logging.warning("⚠️ Só %s ofertas. Mínimo %s.", len(ofertas), MIN_OFERTAS)
            return
        selecionadas = []
        for nicho, p in ofertas:
            try:
                ...
                logging.error("❌ Erro produto: %s", e)
        if len(selecionadas) < MIN_OFERTAS:
            logging.warning("⚠️ Só %s ofertas válidas.", len(selecionadas))
            return
        await enviar_lote(ctx, selecionadas)
        logging.info("========== 🆓 BLOCO FREE ==========")
        e = carregar_estado()
        idx = int(e.get("free_nicho_idx", 0))
        nicho = NICHOS_FREE_ROTA[idx % len(NICHOS_FREE_ROTA)]
        oferta = next((x for x in selecionadas if x["nicho_origem"] == nicho), None)
        if oferta and await enviar_produto(ctx, oferta, FREE_CHAT_ID):
            registrar_historico(oferta["produto_id"])
        else:
            logging.warning("⚠️ Sem oferta FREE para %s", nicho)
        e["free_nicho_idx"] = (idx + 1) % len(NICHOS_FREE_ROTA)
        salvar_estado(e)
        logging.info("========== ✅ CICLO FINALIZADO ==========")
    except Exception as e:
        logging.error("❌ ERRO CICLO: %s", e, exc_info=True)
...
async def ofertas_loop(app):
    ...
    while True:
        try:
            await send_ofertas(type("Contexto", (), {"bot": app.bot})())
        except Exception as e:
            logging.error("❌ Erro loop: %s", e, exc_info=True)
...
def validar_config():
    ...
    if faltam:
        raise RuntimeError("Variáveis ausentes: " + ", ".join(faltam))

def iniciar():
    validar_config()
    logging.info("=" * 45)
    logging.info("🚀 SHOPEE BOT V30 - SEM REPETIÇÃO")
    logging.info("🔄 Ordenação, página e termo variam a cada busca")
    logging.info("📦 Histórico de 30 dias + 1 produto por família")
    logging.info("=" * 45)
    while True:
        try:
