...
def obter_ofertas_shopee():
    ...
    sel = []; est = carregar_estado()
    peca, moto, est = proxima_busca_moto(est)
    its, est = selecionar("Moto", moto, 1, est, True, peca)
    sel.extend([("Moto",x) for x in its])
    conf = {"Casa":2,"Bebê":2,"Eletrônicos":2,"Moda Feminina":1,"Moda Masculina":1}
    for n,q in conf.items():
        for _ in range(q):
            t, est = proximo_termo(n, est)
            its, est = selecionar(n, t, 1, est)
            sel.extend([(n,x) for x in its])
    salvar_estado(est)
...
def montar_tg(nome, preco, vendas, nota, comissao, link, lk_whats):
...
async def ciclo(ctx):
    try:
        ...
        if len(ofertas)<MIN_OFERTAS: logging.warning("⚠️ Poucas ofertas: %s", len(ofertas)); return
        ...
        for nicho, p in ofertas:
            try:
                ...
                txt_tg = montar_tg(titulo,prc,vnd,nt,comissao,link,lk_whats)
        ...
        if len(enviados)<MIN_OFERTAS: return
